jQuery(document).ready(function($) {
    ...
    var url = 'http://dopa-services.jrc.ec.europa.eu/services/ibex/ehabitat/get_wdpa_lc_stats_glob2005?wdpaid=' + $paid;
    $.ajax({
        url: url,
        dataType: 'json',
        success: function(d) {
            if (d.metadata.recordCount == 0) {
                jQuery('#glob2005-chart.rest-good').switchClass( "rest-good", "rest-bad", 100 );
                jQuery('#glob2005-chart').append('There is no GLOB2005 data for ' + $paname)
            } else {
                var obj = {};
                var colors_array = [];
                var obj_array = [];
                ...
                $('#glob2005-chart').highcharts({
                    ...
                    title: {
                        text: 'GlobCover 2005 stats for ' + $paname
                    },
                    legend: {
                        floating: true,
                        width: 500,
        ...
        error: function() {
			jQuery('#glob2005-chart.rest-good').switchClass( "rest-good", "rest-error", 100 );
            jQuery('#glob2005-chart').append('The glob2005 services are down.')
        }
    });
});